...
def _cause_etrangere_label(facts: ExtractedFacts, mode_options: Dict[str, bool], retrieved: List[RetrievedSource], raw_text: str = "") -> str:
    ...
    method = assess_methodology(facts, retrieved, raw_text)
    kind = method.entretien_kind if method.defaut_entretien_caracterise else ""
...
def decide(facts: ExtractedFacts, retrieved: List[RetrievedSource], mode_options: Dict[str, bool] | None = None, forced_vat: float | None = None, raw_text: str = "") -> DecisionResult:
    ...
    if facts.mentions_safety or visual_safety:
    ...
    water_context = (facts.mentions_humidity_or_water and not facts.mentions_mold_condensation) or bool(top_ids & {"DOUCHE_ZERO_RESSAUT", "PLOMBERIE_RESEAUX", "ETANCHEITE_TOITURE_TERRASSE_BALCON", "FACADE_INFILTRATION"})
    ...
    if decision_code == "GARANTIE_INF_TM_A_VALIDER" or red_flags or garantie_score < 80 or amount_estimated:
