...
def build_qualification_view(
    ...
    if is_ess_roof:
        ...
        description_lines = [
            _declared(f"Dommage déclaré : {facts.declared_damage or 'fuite localisée sur organe de plomberie'}."),
            _declared(f"Réception : {facts.reception_date}.") if facts.reception_date else _missing("date de réception à confirmer."),
            _analysis(f"Élément affecté : {method.element_affecte}."),
        ]
        technical_lines = [
            _analysis("Démarche : croisement corps d'état / pathologie / fiche entretien / âge du sinistre."),
            _analysis(method.entretien_rationale),
            _analysis("État actuel : fuite localisée d'un organe soumis à usure, sans dommage étendu ni impropriété mentionnée à ce stade."),
        ]
        warranty_lines = [
            _analysis("Avis proposé : non-garantie."),
            _analysis("À partir de la 3e année, le maintien ou remplacement des organes de plomberie soumis à usure relève de l'entretien normal, si aucun défaut constructif n'est objectivé."),
            _analysis("Garantie obligatoire DO non mobilisable, sous validation humaine."),
        ]
        remedy_lines = [
            _analysis("Solution : intervention d'entretien / remplacement localisé de la pièce fuyarde ; vérifier l'absence de dommage en local inférieur."),
            _analysis(f"Quantum : {decision.montant_ttc:,.0f} € TTC, non indemnisé au titre de la DO.".replace(",", " ")) if decision.montant_ttc else _missing("quantum à confirmer par devis d'entretien."),
            _analysis("Méthode : pas de pré-chiffrage travaux DO dès lors que l'entretien/remplacement courant constitue l'orientation principale."),
        ]
    ...
    elif (('is_maintenance_roof' in locals() and is_maintenance_roof) or ('is_shower_mastic_maintenance' in locals() and is_shower_mastic_maintenance) or (is_mold and not is_luminaire)):
