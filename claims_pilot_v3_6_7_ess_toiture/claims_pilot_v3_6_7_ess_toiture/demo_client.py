...
st.set_page_config(page_title="ClaimsPilot V3.6.7 - Démo Qualification", page_icon="🧭", layout="wide", initial_sidebar_state="collapsed")
...
def _reset_analysis() -> None:
    ...
    La V3.6.7 privilégie la sécurité : le widget d'upload est régénéré,
...
def _run_pipeline(files, manual_text: str, forced_vat: float | None) -> Dict[str, Any]:
    ...
    for uf in files or []:
        ...
        if Path(uf.name).suffix.lower() in {".jpg", ".jpeg", ".png", ".webp"}:
...
def _client_markdown(result: Dict[str, Any]) -> str:
    ...
    d = result["decision"]
    lines = [
        "# Réponse ClaimsPilot V3.6.7 — Qualification",
...
st.markdown(
    """
    <div class="hero">
      <h1>ClaimsPilot V3.6.7 — Qualification</h1>
      <p>Démo de qualification DO à partir d'une déclaration et de ses pièces. La V3.6.7 est volontairement stateless et recentrée sur la démarche expertale : aucune ancienne analyse n'est conservée en mémoire.</p>
...
# La V3.6.7 évite volontairement st.form : sur Streamlit Cloud, le couple
...
st.download_button("Télécharger la réponse (.md)", md.encode("utf-8"), file_name="reponse_claimspilot_v362.md", mime="text/markdown", use_container_width=True)
