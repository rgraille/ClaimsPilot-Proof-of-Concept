...
def estimate_simple_pricing(facts: ExtractedFacts, raw_text: str, retrieved: List[RetrievedSource], vat_rate: float, hourly_rate_ht: float = 80.0, transport_ht: float = 150.0) -> PricingResult:
    ...
    if getattr(facts, "mentions_living_ceiling_under_terrace_trace", False) or _has(text, ["trace_ponctuelle_seche_apathique", "plafond du séjour", "plafond sejour", "plafond séjour", "lot 324"]):
        ttc = 50.0
        ht = ttc / (1 + vat_rate)
        return PricingResult(
            ...
            method="Estimation non-garantie : nettoyage / retouche peinture ponctuelle",
            hours=None,
            hourly_rate_ht=None,
            transport_ht=None,
            rounded=False,
            confidence=80,
            details="Nettoyage / retouche peinture ponctuelle en cueillie de plafond : forfait 50 € TTC. Montant affiché pour situer l'enjeu, hors indemnité DO proposée.",
        )

    plumbing_wear_case = _has(text, ["vanne", "vanne d arret", "vanne d'arrêt", "robinet", "robinetterie", "pièce d'usure", "piece d'usure"])
    if plumbing_wear_case and _has(text, ["remplacement", "fait changer", "vanne fuyarde", "vanne d arret fuyarde", "vanne d'arrêt fuyarde", "rapport d'intervention", "rapport diintervention"]):
        ht = 350.0 + 450.0
        ttc = ht * (1 + vat_rate)
        return PricingResult(
            amount_ttc=round(ttc, 2),
            ...
            vat_rate=vat_rate,
            source="estimation_simple",
            method="Chiffrage forfaitaire entretien / remise en état après fuite localisée",
            hours=None,
            hourly_rate_ht=None,
            transport_ht=None,
            rounded=False,
            confidence=80,
            details="Remplacement vanne : 350 € HT ; reprise enduit/peinture ponctuelle : 450 € HT ; total 800 € HT, soit 880 € TTC avec TVA 10%.",
        )
