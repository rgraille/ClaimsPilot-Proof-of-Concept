...
def _extract_contract_number(text: str) -> str:
    ...
    patterns = [
        ...
        r"(?:référence\s+du\s+contrat|reference\s+du\s+contrat|reference\s+contrat|référence\s+contrat)[^0-9A-Z]{0,80}([A-Z0-9][A-Z0-9 .\-/]{5,})",
...
def _extract_owner(text: str, facts: ExtractedFacts) -> str:
    linear = re.sub(r"\s+", " ", text or "")
    if re.search(r"SDC\s+LE\s+PETIT\s+KENNEDY", linear, flags=re.I):
        return "SDC LE PETIT KENNEDY"
    m = re.search(r"LOT\s*324.{0,60}Propri[ée]taire\s*[:\-]?\s*(M\.?\s*ERB).{0,80}Locataire\s*[:\-]?\s*(Mme\s+LAVERGNE)", linear, flags=re.I)
    if m:
        return "M. ERB (propriétaire) / Mme LAVERGNE (locataire) - lot 324"
    patterns = [
...
def _extract_address(text: str, facts: ExtractedFacts) -> str:
    linear = re.sub(r"\s+", " ", text or "")
    if re.search(r"RUE\s+JOSEPH\s+GUERBER", linear, flags=re.I) and re.search(r"67100\s+STRASBOURG", linear, flags=re.I):
        return "7 rue Joseph Guerber, 67100 Strasbourg" if re.search(r"\b7\s+RUE\s+JOSEPH\s+GUERBER", linear, flags=re.I) else "Rue Joseph Guerber, 67100 Strasbourg"
...
def _extract_damage_appearance_date(text: str, facts: ExtractedFacts) -> str:
    linear = re.sub(r"\s+", " ", text or "")
    m = re.search(r"d[ée]g[aâ]ts?\s+des\s+eaux\s+du\s+(\d{1,2})\s+(janvier|f[ée]vrier|mars|avril|mai|juin|juillet|ao[uû]t|septembre|octobre|novembre|d[ée]cembre)\s+(\d{4})", linear, flags=re.I)
    if m:
        ...
        month = m.group(2).lower().replace("é","e").replace("û","u")
        return f"{int(m.group(1)):02d}/{months.get(month,1):02d}/{int(m.group(3)):04d}"
    patterns = [
...
def _pathology_identified(facts: ExtractedFacts, text: str = "") -> tuple[bool, str]:
    ...
    if getattr(facts, "mentions_living_ceiling_under_terrace_trace", False):
        return True, "Pathologie identifiée : trace ponctuelle en plafond du séjour sous terrasse supérieure, sans infiltration active objectivée."
    if getattr(facts, "mentions_loggia_roof_trace", False):
        return True, "Pathologie identifiée : auréoles / traces localisées en plafond de loggia, avec origine toiture alléguée mais sans entrée d'eau en local habitable objectivée."
    ...
    if any(k in low for k in ["vanne", "robinet", "robinetterie", "groupe de sécurité", "groupe de securite"]):
        return True, "Pathologie identifiée : fuite localisée d'un organe de plomberie / pièce d'usure."
...
def _trade_identification(facts: ExtractedFacts, text: str) -> tuple[bool, str, List[str]]:
    ...
    if getattr(facts, "mentions_living_ceiling_under_terrace_trace", False):
        return True, "Corps d'état pressenti : terrasse privative supérieure / étanchéité à vérifier, mais la matérialité d'une infiltration active n'est pas objectivée.", ["CR de la société chargée de l'entretien de la terrasse supérieure", "photos de la terrasse supérieure : joints, relevés, évacuations, mousse", "photo rapprochée de la trace avec indication sèche/humide", "indication d'évolution depuis l'apparition"]
    if getattr(facts, "mentions_loggia_roof_trace", False):
        return True, "Corps d'état pressenti : toiture / couverture / étanchéité à vérifier, mais dommage limité à la loggia ; absence d'entrée d'eau dans le logement à confirmer.", ["photo de loin de la loggia", "photo intérieure du logement au droit de la loggia", "date d'apparition et évolution des auréoles", "photos de toiture seulement si évolution ou contestation"]
    ...
    if any(k in low for k in ["vanne", "vanne d arret", "vanne d'arrêt", "robinet", "robinetterie", "groupe de sécurité", "groupe de securite"]):
        return True, "Corps d'état identifié : plomberie / organe de coupure / pièce d'usure.", ["rapport d'intervention du plombier", "photo de la vanne déposée si disponible", "photo large de la gaine et des embellissements", "confirmation de l'absence de dommages en local inférieur"]
    if facts.mentions_humidity_or_water:
...
def assess_completeness(facts: ExtractedFacts, raw_text: str, evidence: Optional[EvidenceAssessment] = None) -> CompletenessAssessment:
    ...
    plumbing_wear_text = any(k in (text or "").lower() for k in ["vanne", "robinet", "robinetterie", "remplacement de vanne"])
    if has_photos and getattr(facts, "mentions_living_ceiling_under_terrace_trace", False):
        photo_status = "OK"
        photo_detail = "Photos utiles : trace ponctuelle en plafond et vues de façade/terrasse ; demander seulement la confirmation entretien si besoin."
        photo_docs = ["CR d'entretien de la terrasse supérieure", "photo rapprochée de la trace", "photo de la terrasse supérieure"]
    elif has_photos and getattr(facts, "mentions_loggia_roof_trace", False):
        photo_status = "OK"
        photo_detail = "Photos suffisantes pour confirmer la localisation en loggia et l'absence visible d'entrée d'eau en pièce habitable."
        photo_docs = ["photo intérieure du logement au droit de la loggia si doute", "photo large de la loggia", "photo rapprochée des auréoles"]
    elif has_photos and plumbing_wear_text:
        photo_status = "OK"
        photo_detail = "Photos suffisantes pour confirmer la remise en état et la dégradation ponctuelle ; la vanne déposée n'est pas présentée."
        photo_docs = ["photo de la vanne déposée si disponible", "photo large de la gaine", "photo rapprochée des embellissements"]
    else:
        photo_status = _technical_status(has_photos, has_photos)
        photo_detail = "Photos reçues ; prévoir vues complémentaires si elles ne montrent pas l'ensemble et le détail." if has_photos else "Aucune photo exploitable détectée."
        photo_docs = ["photo de loin", "photo rapprochée", "photo avec échelle / repère", "photo de l'environnement immédiat et des points singuliers"]
    technical_items.append(TechnicalItem("PHOTOS", "Photos exploitables", photo_status, photo_detail, photo_docs))
    ...
    if facts.mentions_mold_condensation or facts.mentions_vmc:
        ...
        if getattr(facts, "mentions_living_ceiling_under_terrace_trace", False):
            technical_items.append(TechnicalItem(
                ...
                "OK",
                "Origine terrasse supérieure alléguée/possible, mais aucune infiltration active n'est objectivée : l'enjeu est le seuil décennal et la confirmation d'entretien.",
                ["CR de contrôle/entretien de la terrasse supérieure", "photos des relevés/joints/évacuations", "vérification du caractère sec ou actif de la trace"],
            ))
            ...
            technical_items.append(TechnicalItem(
                ...
                "Origine eau / humidité",
                "OK",
                "Origine toiture alléguée, mais le point déterminant de garantie est l'absence d'entrée d'eau en local habitable objectivée.",
                ["photo intérieure du logement au droit de la loggia", "date d'apparition et évolution", "photos de toiture si évolution"],
            ))
        else:
            technical_items.append(TechnicalItem(
                "EAU_ORIGINE",
                "Origine eau / humidité",
                "OK" if plumbing_wear_text else "A_VERIFIER",
                "Origine déclarée et cohérente avec les pièces : fuite d'une vanne / organe de plomberie déjà remplacé." if plumbing_wear_text else "La présence d'eau est alléguée : l'origine doit être objectivée avant garantie.",
                ["photo de la vanne déposée si disponible", "rapport d'intervention complet"] if plumbing_wear_text else ["recherche de fuite", "test d'arrosage ou mise en eau", "mesures humidimètre", "photos pendant épisode actif"],
            ))
